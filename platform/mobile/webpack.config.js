...
const { getWebpackTools, getWorkspaces, getNohoist } = require("react-native-monorepo-tools");
...
function sharedModules() {
    ...
    Object.keys(require("./package.json").dependencies).forEach((lib) => {
        if (lib !== "react-native-web" || lib !== "react-dom") {
            oLib[lib] = {
                singleton: true,
                eager: true,
                requiredVersion: require("./package.json").dependencies[lib]
                // "import": true
            };
        }
    });
...
const webpackConfig =  {
    ...
    resolve: {
         ...
         */
        // alias: {
        //     '@superlit/test-miniapp': '/Users/manojsadhu/WorkSpace/experiment/superlit-core/test-miniapp/index.tsx',
        // },
...
nohoistLibNames.forEach((nohoistLibName) => {
    nohoistAlias[nohoistLibName] = path.resolve(cwd, `./node_modules/${nohoistLibName}`);
});
...
const babelLoader = webpackConfig.module.rules.find(rule => rule.use === 'babel-loader')
